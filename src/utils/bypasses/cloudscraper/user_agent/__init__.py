import json, os, random, re, sys, ssl
...
class User_Agent():
    ...
    def filterAgents(self, user_agents):
        ...
        if self.mobile and self.platform in user_agents['mobile'] and user_agents['mobile'][self.platform]: filtered.update(user_agents['mobile'][self.platform])
        if self.desktop and self.platform in user_agents['desktop'] and user_agents['desktop'][self.platform]: filtered.update(user_agents['desktop'][self.platform])
        return filtered
    ...
    def loadUserAgent(self, *args, **kwargs):
        ...
        if self.custom:
            ...
            if not self.platform: self.platform = random.SystemRandom().choice(self.platforms)
