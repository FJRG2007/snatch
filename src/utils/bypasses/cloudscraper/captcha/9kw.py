...
try: import polling
except ImportError: raise ImportError("Please install the python module 'polling' via pip or download it from " "https://github.com/justiniso/polling/")
from ..exceptions import (CaptchaException, CaptchaServiceUnavailable, CaptchaAPIError, CaptchaTimeout, CaptchaParameter, CaptchaBadJobID)
from . import Captcha
...
class captchaSolver(Captcha):
    ...
    def checkErrorStatus(response):
        if response.status_code in [500, 502]: raise CaptchaServiceUnavailable(f'9kw: Server Side Error {response.status_code}')
        ...
        if response.text.startswith('{') and response.json().get('error'): raise CaptchaAPIError(error_codes.get(int(response.json().get('error'))))
        else:
            error_code = int(re.search(r'^00(?P<error_code>\d+)', response.text).groupdict().get('error_code', 0))
            if error_code: raise CaptchaAPIError(error_codes.get(error_code))

    def requestJob(self, jobID):
        if not jobID: raise CaptchaBadJobID("9kw: Error bad job id to request against.")
        ...
        if response: return response.json().get('answer')
        else: raise CaptchaTimeout("9kw: Error failed to solve.")

    def requestSolve(self, captchaType, url, siteKey):
        def _checkRequest(response):
            if response.ok and response.text.startswith('{') and response.json().get('captchaid'): return response
            self.checkErrorStatus(response)
            return None
        ...
        if response: return response.json().get('captchaid')
