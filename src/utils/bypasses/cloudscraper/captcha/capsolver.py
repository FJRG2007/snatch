...
class captchaSolver(Captcha):
    ...
    def checkErrorStatus(response, fnct):
        if response.status_code in [500, 502]: raise CaptchaServiceUnavailable(f'CapSolver: Server Side Error {response.status_code}')
        try: rPayload = response.json()
        except Exception: return
        if rPayload.get('errorDescription', False) and 'Current system busy' not in rPayload['errorDescription']: raise CaptchaAPIError(f"CapSolver -> {fnct} -> {rPayload.get('errorDescription')}")
    ...
    def requestJob(self, jobID):
        ...
        if response:
            try:
                ...
                if 'token' in rPayload: return rPayload['token']
                else: return rPayload['gRecaptchaResponse']
            except Exception: pass
    ...
    def getCaptchaAnswer(self, captchaType, url, siteKey, captchaParams):
        ...
        except polling2.TimeoutException: raise CaptchaTimeout(f"CapSolver: Captcha solve (task ID: {jobID}) took to long.")
