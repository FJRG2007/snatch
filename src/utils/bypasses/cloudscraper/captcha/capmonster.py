...
class captchaSolver(Captcha):
    ...
    def checkErrorStatus(response):
        if response.status_code in [500, 502]: raise CaptchaServiceUnavailable(f'CapMonster: Server Side Error {response.status_code}')
    ...
    def requestJob(self, taskID):
        if not taskID: raise CaptchaBadJobID('CapMonster: Error bad task id to request Captcha.')
    ...
    def requestSolve(self, captchaType, url, siteKey):
        ...
        else: raise CaptchaBadJobID('CapMonster: Error no task id was returned.')

    def getCaptchaAnswer(self, captchaType, url, siteKey, captchaParams):
        taskID = None
        if not captchaParams.get('clientKey'): raise CaptchaParameter("CapMonster: Missing clientKey parameter.")
