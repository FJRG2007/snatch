...
from ..exceptions import (CaptchaException, CaptchaServiceUnavailable, CaptchaTimeout, CaptchaParameter, CaptchaBadJobID, CaptchaReportError)
...
class captchaSolver(Captcha):
    ...
    def checkErrorStatus(response):
        ...
        if response.status_code in errors: raise CaptchaServiceUnavailable(errors.get(response.status_code))
    ...
    def login(self, username, password):
        ...
        def _checkRequest(response):
            if response.ok:
                if response.json().get('is_banned'): raise CaptchaServiceUnavailable('DeathByCaptcha: Your account is banned.')
                if response.json().get('balanace') == 0: raise CaptchaServiceUnavailable('DeathByCaptcha: insufficient credits.')
                return response
    ...
    def reportJob(self, jobID):
        if not jobID: raise CaptchaBadJobID("DeathByCaptcha: Error bad job id to report failed reCaptcha.")
        ...
        else: raise CaptchaReportError("DeathByCaptcha: Error report failed reCaptcha.")

    def requestJob(self, jobID):
        if not jobID: raise CaptchaBadJobID("DeathByCaptcha: Error bad job id to request reCaptcha.")

        def _checkRequest(response):
            if response.ok and response.json().get('text'): return response
            self.checkErrorStatus(response)
            return None
    ...
    def requestSolve(self, captchaType, url, siteKey):
        ...
        if response: return response.json().get('captcha')
        else: raise CaptchaBadJobID('DeathByCaptcha: Error no job id was returned.')

    def getCaptchaAnswer(self, captchaType, url, siteKey, captchaParams):
        jobID = None
        for param in ['username', 'password']:
            if not captchaParams.get(param): raise CaptchaParameter(f"DeathByCaptcha: Missing '{param}' parameter.")
