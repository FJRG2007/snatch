...
from ..exceptions import (CaptchaServiceUnavailable, CaptchaAPIError, CaptchaTimeout, CaptchaParameter, CaptchaBadJobID)
try: import polling2
...
class captchaSolver(Captcha):
    ...
    def checkErrorStatus(response):
        ...
        if payload['errorId'] >= 1:
            ...
            else: raise CaptchaAPIError(payload['errorCode'])

    def requestJob(self, taskID):
        if not taskID: raise CaptchaBadJobID('anticaptcha: Error bad task id to request Captcha.')
        ...
        else: raise CaptchaTimeout("anticaptcha: Error failed to solve Captcha.")
    ...
    def requestSolve(self, captchaType, url, siteKey):
        ...
        if self.proxy: data['task'].update(self.proxy)
        else: data['task']['type'] = f"{data['task']['type']}Proxyless"
    ...
    def getCaptchaAnswer(self, captchaType, url, siteKey, captchaParams):
        ...
        try:
            ...
            except polling2.TimeoutException: raise CaptchaTimeout("anticaptcha: Captcha solve took to long and also failed " f"reporting the task with task id {taskID}.")
            raise CaptchaTimeout("anticaptcha: Captcha solve took to long to execute " f"task id {taskID}, aborting.")
