...
from ..exceptions import (CaptchaServiceUnavailable, CaptchaAPIError, CaptchaTimeout, CaptchaParameter, CaptchaBadJobID, CaptchaReportError)

try: import polling2
except ImportError: raise ImportError("Please install the python module 'polling2' via pip")
...
class captchaSolver(Captcha):
    ...
    def checkErrorStatus(response, request_type):
        if response.status_code in [500, 502]: raise CaptchaServiceUnavailable(f'2Captcha: Server Side Error {response.status_code}')
        ...
        if rPayload.get('status') == 0 and rPayload.get('request') in errors.get(request_type): raise CaptchaAPIError(f"{rPayload['request']} {errors.get(request_type).get(rPayload['request'])}")
    ...
    def reportJob(self, jobID):
        ...
        def _checkRequest(response):
            ...
            if response.ok and response.json().get('status') == 1: return response
            return None
    ...
    def requestJob(self, jobID):
        ...
        if response: return response.json().get('request')
        else: raise CaptchaTimeout("2Captcha: Error failed to solve Captcha.")

    def requestSolve(self, captchaType, url, siteKey):
        def _checkRequest(response):
            self.checkErrorStatus(response, 'in.php')
            if response.ok and response.json().get("status") == 1 and response.json().get('request'): return response
            return None
        ...
        if response: return response.json().get('request')
        else: raise CaptchaBadJobID('2Captcha: Error no job id was returned.')
    
    def getCaptchaAnswer(self, captchaType, url, siteKey, captchaParams):
        jobID = None
        if not captchaParams.get('api_key'): raise CaptchaParameter("2Captcha: Missing api_key parameter.")
        ...
        try:
            ...
            except polling2.TimeoutException: raise CaptchaTimeout(f"2Captcha: Captcha solve took to long and also failed reporting the job the job id {jobID}.")
