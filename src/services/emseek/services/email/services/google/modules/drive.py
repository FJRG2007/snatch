...
from ..objects.base import DriveExtractedUser
from ..apis.drive import DriveHttp
from ..apis.clientauthconfig import ClientAuthConfigHttp
from .. import globals as gb
from ..helpers import auth
from ..helpers.drive import get_comments_from_file, get_users_from_file
from ..knowledge import drive as drive_knownledge

import httpx, inspect, humanize, inflection
from typing import *
from datetime import timedelta

def show_user(user: DriveExtractedUser):
    if user.name: print(f"Full name : {user.name}")
    else: gb.rc.print("Full name : -", style="bright_black")
    print(f"Email : {user.email_address}")
    if user.gaia_id: print(f"Gaia ID : {user.gaia_id}")
    else: gb.rc.print("Gaia ID : -", style="bright_black")
    if user.is_last_modifying_user: print("[+] Last user to have modified the document !")
...
async def hunt(as_client: httpx.AsyncClient, file_id: str, json_file: bool=Path):
    ...
    if not file_found: exit("[-] The file wasn't found.")
    ...
    if file.md5_checksum: print(f"MD5 Checksum : {file.md5_checksum}")
    if file.file_size: print(f"{'Folder' if is_folder else 'File'} size : {humanize.naturalsize(file.file_size)}")
    if file_type: gb.rc.print(f"\nType : {file_type} [italic]\[{file.mime_type}][/italic]")
    else: print(f"\nMime Type : {file.mime_type}")
    if is_folder: print(f"Folder link :\n=> {file.alternate_link}")
    else: print(f"Download link :\n=> {file.alternate_link}")
    ...
    if file.source_app_id:
        ...
        if brand_found:
            ...
            if brand.home_page_url: print(f"Home page : {brand.home_page_url}")
            else: gb.rc.print(f"Home page : [italic][bright_black]Not found.[/italic][/bright_black]")
        else: gb.rc.print("Not found.", style="italic")
    else: gb.rc.print("No source application.", style="italic")
    if file.image_media_metadata.height and file.image_media_metadata.width:
        ...
        if isinstance((data := file.image_media_metadata.rotation), int): print(f"Rotation : {data}")
    if file.video_media_metadata.height and file.video_media_metadata.width:
        ...
        if (data := file.video_media_metadata.duration_millis): print(f"Duration : {humanize.precisedelta(timedelta(milliseconds=int(file.video_media_metadata.duration_millis)))}")
    gb.rc.print("\n📂 Parents\n", style="gold3")
    if file.parents:
