...
class GHuntCreds(SmartObj):
    ...
    def __init__(self, creds_path: str = "") -> None:
        ...
        if not creds_path:
            ...
            if not ghunt_folder.is_dir(): ghunt_folder.mkdir(parents=True, exist_ok=True)
    ...
    def load_creds(self, silent=False) -> None:
        ...
        if Path(self.creds_path).is_file():
            ...
            except Exception: raise GHuntInvalidSession("Stored session is corrupted.")
        else: return terminal("e", "Invalid credentials for Google.", exitScript=True)
        if not self.are_creds_loaded(): return terminal("e", "Invalid credentials for Google.", exitScript=True)
        if not silent: print("[+] Stored session loaded !")
