...
class GAPI(SmartObj):
    ...
    def _load_api(self, creds: GHuntCreds, headers: Dict[str, str]):
        if not creds.are_creds_loaded(): raise GHuntInsufficientCreds(f"This API requires a loaded GHuntCreds object, but it is not.")
        if not is_headers_syntax_good(headers): raise GHuntCorruptedHeadersError(f"The provided headers when loading the endpoint seems corrupted, please check it : {headers}")
        if self.authentication_mode == "oauth": self.gen_token_lock = asyncio.Semaphore(1)

        cookies = {}
        if self.authentication_mode in ["sapisidhash", "cookies_only"] and not (cookies := creds.cookies): raise GHuntInsufficientCreds(f"This endpoint requires the cookies in the GHuntCreds object, but they aren't loaded.")
        if (key_name := self.require_key):
            if not (api_key := get_api_key(key_name)): raise GHuntInsufficientCreds(f"This API requires the {key_name} API key in the GHuntCreds object, but it isn't loaded.")
            if not self.key_origin: self.key_origin = get_origin_of_key(key_name)
        ...
        if self.authentication_mode == "sapisidhash":
            if not (sapisidhash := creds.cookies.get("SAPISID")): raise GHuntInsufficientCreds(f"This endpoint requires the SAPISID cookie in the GHuntCreds object, but it isn't loaded.")
    ...
    def _load_endpoint(self, endpoint_name: str,
        ...
        if endpoint_name in self.loaded_endpoints: return
    ...
    async def _query(self, as_client: httpx.AsyncClient, verb: str, endpoint_name: str, base_url: str, params: Dict[str, Any], data: Any, data_type: str) -> httpx.Response:
        ...
        if verb == "GET": req = await as_client.get(f"{self.scheme}://{self.hostname}{base_url}", params=params, headers=headers, cookies=endpoint.cookies)
        elif verb == "POST":
            if data_type == "data": req = await as_client.post(f"{self.scheme}://{self.hostname}{base_url}", params=params, data=data, headers=headers, cookies=endpoint.cookies)
            elif data_type == "json": req = await as_client.post(f"{self.scheme}://{self.hostname}{base_url}", params=params, json=data, headers=headers, cookies=endpoint.cookies)
            else: raise GHuntUnknownRequestDataTypeError(f"The provided data type {data_type} wasn't recognized by ...")
        else: raise GHuntUnknownVerbError(f"The provided verb {verb} wasn't recognized by ...")
        return req
...
class Parser(SmartObj):
    def _merge(self, obj) -> any:
        ...
        def recursive_merge(obj1, obj2, module_name: str) -> any:
            ...
            return obj1
        if not get_class_name(obj).startswith(get_class_name(self)): raise GHuntObjectsMergingError("The two objects being merged aren't from the same class.")
        self = recursive_merge(self, obj, self.__module__)