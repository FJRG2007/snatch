...
from ..errors import *
from ..helpers.utils import is_default_profile_pic, unicode_patch
...
class PersonGplusExtendedData(Parser):
    ...
    def _scrape(self, gplus_data):
        ...
        if (isEnterpriseUser := gplus_data.get("isEnterpriseUser")): self.isEntrepriseUser = isEnterpriseUser
...
class PersonExtendedData(Parser):
    ...
    def _scrape(self, extended_data: Dict[str, any]):
        if (dynamite_data := extended_data.get("dynamiteExtendedData")): self.dynamiteData._scrape(dynamite_data)
        if (gplus_data := extended_data.get("gplusExtendedData")): self.gplusData._scrape(gplus_data)
...
class PersonPhoto(Parser):
    ...
    async def _scrape(self, as_client: httpx.AsyncClient, photo_data: Dict[str, any], photo_type: str):
        if photo_type == "profile_photo":
            ...
            if (isDefault := photo_data.get("isDefault")): self.isDefault = isDefault
...
class PersonProfileInfo(Parser):
    ...
    def _scrape(self, profile_data: Dict[str, any]):
        if "ownerUserType" in profile_data: self.userTypes += profile_data.get("ownerUserType")
...
class PersonSourceIds(Parser):
    ...
    def _scrape(self, source_ids_data: Dict[str, any]):
        if (timestamp := source_ids_data.get("lastUpdatedMicros")): self.lastUpdated = datetime.utcfromtimestamp(float(timestamp[:10]))
...
class PersonInAppReachability(Parser):
    ...
    def _scrape(self, apps_data, container_name: str):
        for app in apps_data:
            if app["metadata"]["container"] == container_name: self.apps.append(app["appType"].title())

class PersonContainers(dict): pass
...
class Person(Parser):
    ...
    async def _scrape(self, as_client: httpx.AsyncClient, person_data: Dict[str, any]):
        ...
        if (extended_data := person_data.get("extendedData")): self.extendedData._scrape(extended_data)