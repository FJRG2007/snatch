...
from ..objects.apis import Parser
...
class PlayerProfile(Parser):
    ...
    def _scrape(self, player_data: Dict[str, any]):
        ...
        if (last_played_app_data := player_data.get("lastPlayedApp")): self.last_played_app._scrape(last_played_app_data)
        if (profile_settings_data := player_data.get("profileSettings")): self.profile_settings._scrape(profile_settings_data)
        if (experience_data := player_data.get("experienceInfo")): self.experience_info._scrape(experience_data)
...
class PlayGame(Parser):
    ...
    def _scrape(self, game_data: Dict[str, any]):
        if (games_data := game_data.get("gamesData")): self.game_data._scrape(games_data)
        if (market_data := game_data.get("marketData")): self.market_data._scrape(market_data)
        self.formatted_last_played_time = game_data.get("formattedLastPlayedTime")
        if (timestamp := game_data.get("lastPlayedTimeMillis")): self.last_played_time_millis = datetime.utcfromtimestamp(float(timestamp[:10]))
...
class PlayGameMarketInstance(Parser):
    ...
    def _scrape(self, instance_data: Dict[str, any]):
        ...
        if (rating_data := instance_data.get("rating")): self.rating._scrape(rating_data)
        if (timestamp := instance_data.get("lastUpdatedTimestampMillis")): self.last_updated_timestamp_millis = datetime.utcfromtimestamp(float(timestamp[:10]))
...
class PlayGameData(Parser):
    ...
    def _scrape(self, game_data: Dict[str, any]):
        ...
        if (category_data := game_data.get("category")): self.category._scrape(category_data)
        ...
        if (instances_data := game_data.get("instances")):
            for instance_data in instances_data:
                ...
                instance._scrape(instance_data)
                self.instances.append(instance)
        if (timestamp := game_data.get("lastUpdatedTimestamp")): self.last_updated_timestamp = datetime.utcfromtimestamp(float(timestamp[:10]))
        ...
        if (achievements_items_data := achievements_data.get("items")):
            for achievement_item_data in achievements_items_data:
                ...
                for achievement_def in achievements_defs:
                    if achievement_def.id == achievement.id: achievement.definition = achievement_def
