...
class DictToObj:
    ...
    def __setattr__(self, name, value):
        if name == '__dict__':
            ...
            if hasattr(self, '_parent') and hasattr(self, '_path'):
                self._parent._update_config(self._path, value)
            self._save_to_file()
    ...
    def _update_config(self, key, value):
        ...
        self.__dict__[key] = value
        if hasattr(self, '_parent') and hasattr(self, '_path'):
            self._parent._update_config(self._path, self)
        else:
            self._save_to_file()
    ...
    def _save_to_file(self):
        try:
            ...
            terminal("e", "\"config.json\" file not found.")
        except json.JSONDecodeError:
            terminal("e", "Invalid JSON format in \"config.json\" file.")
        ...
        for key, value in updates.items():
            ...
            if key in data:
                if isinstance(data[key], dict) and isinstance(value, dict):
                    self._update_data(data[key], value)
                else:
                    data[key] = value
            else:
                data[key] = value
...
class Config:
    def __init__(self):
        try:
            ...
            terminal("e", f"Missing key {e} in \"config.json\" file.")

    def get_api_key(self, name):
        if name == "HUNTER":
            api_key = os.getenv("HUNTER_API_KEY")
            if len(api_key) < 7:
                return random.choice(data.DEFAULT_API_KEYS["HUNTER"])
            elif len(api_key) >= 7:
                return api_key
            else:
                terminal("e", "Invalid Hunter API Key.", True)
